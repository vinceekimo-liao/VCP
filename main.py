...
def minervini_check(data):
    ...
    try:
        ...
        if len(ma200_slice) < 2 or ma200_slice.iloc[-1] <= ma200_slice.iloc[0]:
            return False
...
def vcp_math_check(data):
    ...
    try:
        ...
        if pd.isna(vol_ma_20.iloc[-1]) or recent_vol > vol_ma_20.iloc[-1] * 0.5:
            return False
        ...
        return {
            ...
            "high_52w": round(float(high.rolling(250).max().iloc[-1]), 2),
            "low_52w": round(float(low.rolling(250).min().iloc[-1]), 2),
        }
    except:
        return False

# ========== 主掃描函數 ==========
def full_scan    """執行完整漏斗篩選"""
    ...
    for sid, df in layer1_results:
        ...
        if result:
            result["symbol"] = sid
            result["quality"] = "A" if result["contractions"] >= 3 and result["volume_ratio"] >= 1.5 else "B" if result["contractions"] >= 2 else "C"
