...
def get_filtered_stock_ids():
  ...
  for attempt in range(max_retries):
    try:
      ...
      info = info[
          ~info['stock_name'].str.contains('權|ETF|存託憑證', na=False)
      ]
...
def minervini_check(data):
  ...
  try:
    ...
    # 1. 均線多頭排列 (50MA > 150MA > 200MA)
    if not (
        c_last > m50_last and m50_last > m150_last and m150_last > m200_last
    ):
      return False, None
    ...
    stage2_info = {
        'symbol': (
            str(data['stock_id'].iloc[0]) if 'stock_id' in data.columns else ''
        ),
...
def vcp_math_check(data):
  ...
  volume = pd.to_numeric(
      _get_col(data, 'Trading_Volume', 'volume', 'Volume'), errors='coerce'
  )
  ...
  try:
    ...
    for p_idx in peaks_idx:
      sub_valleys = [
          ...
          if v_idx > p_idx and (v_idx - p_idx) >= 3
      ]
      if sub_valleys:
        v_idx = sub_valleys[0]
        peak_p = h.iloc[p_idx]
    ...
    if contractions >= 2:
      is_decreasing = pullbacks[-2] > pullbacks[-1]
      if contractions >= 3:
    ...
    if past_close <= 0:
      return None
    pct_change_60d = (c.iloc[-1] - past_close) / past_close * 100
    ...
    return {
        ...
        'pct_change_60d': round(float(pct_change_60d), 2),
        'rs_score': rs,
        'contractions': contractions if vcp_contracting else 0,
        'last_drop_pct': pullbacks[-1] if pullbacks else 0,
        'volume_ratio': round(float(vol_ratio), 2),
        'vdu': vdu_flag,
        'quality': quality,
        'buy_signal': bool(buy_signal),
        'consecutive_days': 0,
        'pullback_history': str(pullbacks),
    }
  except Exception as e:
    print(f'  VCP error: {e}')
    return None
...
def generate_and_send_sepa_vcp_report(results, total_scanned, stage2_list):
  ...
  if not df_stage2.empty:
    df_stage2['industry'] = df_stage2['symbol'].map(
        lambda x: industry_map.get(x, '其他')
    )
    group_counts = (
        df_stage2.groupby('industry').size().reset_index(name='強勢股數量')
    )
    group_counts = group_counts.sort_values('強勢股數量', ascending=False)
    group_counts['族群狀態'] = group_counts['強勢股數量'].apply(
        lambda x: '🔥 領頭族群共振' if x >= 2 else '單兵強勢'
    )
    group_summary_df = group_counts

  market_bull, market_price = get_market_status()
  near_52w_high_count = (
      len([s for s in stage2_list if s.get('near_52w_high')])
      if stage2_list
      else 0
  )
  ...
  breadth_summary = pd.DataFrame([{
      ...
      '最終黃金買點訊號數': (
          len(df_buy_signals) if not df_buy_signals.empty else 0
      ),
      '加權指數價格': market_price,
  ...
  try:
    with pd.ExcelWriter(sepa_excel_filename, engine='openpyxl') as writer:
      breadth_summary.to_excel(
          writer, sheet_name='大盤廣度與健康度簡報', index=False
      )

      if not df_buy_signals.empty:
        df_buy_signals.to_excel(
            writer, sheet_name='SEPA+VCP 最終進場訊號', index=False
        )
      else:
        pd.DataFrame([{'訊息': '今日無最終進場訊號'}]).to_excel(
            writer, sheet_name='SEPA+VCP 最終進場訊號', index=False
        )

      if not group_summary_df.empty:
        group_summary_df.to_excel(
            writer, sheet_name='產業族群共振榜', index=False
        )

      if not df_stage2.empty:
        df_stage2.to_excel(
            writer, sheet_name='SEPA 趨勢範本合格池', index=False
        )
    ...
    with pd.ExcelWriter(vcp_excel_filename, engine='openpyxl') as writer:
      if not df_all_vcp.empty:
        ...
        df_buy_signals.to_excel(
            writer, sheet_name='最終進場訊號', index=False
        )
      else:
    ...
    for fn in [sepa_excel_filename, vcp_excel_filename]:
      if os.path.exists(fn):
        with open(fn, 'rb') as f:
          attachments.append({'filename': fn, 'content': list(f.read())})
    ...
    params = {
        ...
        'to': [EMAIL_RECEIVER],
        'subject': (
            f"🎯 SEPA+VCP 每日量化策略報告 {now_dt.strftime('%Y-%m-%d')} (進場訊號:"
            f' {buy_count} 檔)'
        ),
        'html': html_content,
    ...
    print(
        f"✅ 每日 SEPA+VCP Email 已成功寄送至 {EMAIL_RECEIVER}，Resend ID:"
        f" {response['id']}"
    )
...
def send_email_report(results, total_scanned):
  generate_and_send_sepa_vcp_report(
      results, total_scanned, sepa_stage2_candidates
  )
...
def manual_scanner():
  ...
  for idx, sid in enumerate(stocks, 1):
    ...
    if idx % 100 == 0:
      print(
          f'📊 進度：{idx}/{total}，第一層 Stage2 通過：{layer1_pass}，VCP'
          f" 候選：{len(_manual_scan_status['results'])}"
      )
...
def background_scanner():
  ...
  for idx, sid in enumerate(stocks, 1):
    ...
    if idx % 100 == 0:
      print(
          f'📊 背景掃描進度：{idx}/{total}，Stage2 通過：{layer1_pass}，VCP'
          f' 候選：{len(local_results)}'
      )
...
def build_report(total, results):
  ...
  for i, c in enumerate(sorted_results[:15], 1):
    ...
    msg += (
        f"🔹 <b>{symbol}</b> ({c.get('industry','其他')}) | 價:{c['price']} |"
        f" RS:{c['rs_score']} | 品質:{c['quality']} <a"
        f" href='{yahoo_link}'>📈 Yahoo</a>\n"
    )
  return msg
...
def health():
  ...
  return {
      'status': 'ok',
      'scanning': any_scan_running,
      'requests_last_hour': pending,
  }
...
def full_report():
  ...
  total = (
      _manual_scan_status['total']
      if _manual_scan_status['total']
      else len(get_filtered_stock_ids())
  )
  if not results:
    return HTMLResponse(
        content='<html><body><h2>尚無篩選結果</h2></body></html>'
    )
  sorted_results = sorted(results, key=lambda x: -x['rs_score'])
  html = f"""<html><head><meta charset='utf-8'><title>SEPA+VCP 完整報告</title>
    ...
    <table><tr><th>代號</th><th>產業</th><th>股價</th><th>樞買點</th><th>漲跌%</th><th>RS</th><th>收縮次數</th><th>最後回檔%</th><th>量比</th><th>品質</th><th>進場訊號</th><th>Yahoo</th></tr>"""
  for c in sorted_results:
    buy_icon = '✅' if c.get('buy_signal') else '❌'
    ind = c.get('industry', '其他')
    pivot = c.get('pivot_high', 0.0)
    last_drop = c.get('last_drop_pct', 0.0)
    html += f"<tr><td>{c['symbol']}</td><td>{ind}</td><td>{c['price']}</td><td>{pivot}</td><td>{c['change_pct']:+.2f}%</td><td>{c['rs_score']}</td><td>{c['contractions']}</td><td>{last_drop}%</td><td>{c['volume_ratio']}</td><td>{c['quality']}</td><td>{buy_icon}</td><td><a href='https://tw.stock.yahoo.com/quote/{c['symbol']}' target='_blank'>Yahoo</a></td></tr>"
...
def get_trade_signals():
  ...
  return convert_numpy({
      ...
      'entry_tips': [
          '⚠️ 開盤觀察 (9:00-10:00)：若開盤即跌破樞紐低點或前日最低價 → 刪除',
          '⚠️ 若開盤跳空大漲 (>3%~5%) → 觀望不追（已過度延伸）',
          '✅ 若開盤平穩或小幅突破樞買點 → 10:00 後量能確認進場',
          '✅ 進場價：不高於樞買點 +3%',
          '✅ 倉位：試倉 5%~10%，獲利後才加碼',
      ],
      'exit_tips': [
          '停利：+5%~+10% 分批獲利，或持有後評估移動停利 (10MA/20MA)',
          '停損：硬性 5%~7% 或跌破關鍵樞紐低點',
          '時間停損：5 天未達 +3% 即離場觀望',
      ],
  })
